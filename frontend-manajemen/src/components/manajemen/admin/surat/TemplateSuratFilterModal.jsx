...
import { Filter as FilterIcon, X, ListFilter, RotateCcw, Check, GraduationCap, BadgeCheck, Image as ImageIcon } from "lucide-react";
...
const asetOptions = [
  { key: "logo", label: "Punya Logo" },
  { key: "ttd", label: "Punya Tanda Tangan" },
  { key: "stempel", label: "Punya Stempel" },
];
...
const Section = ({ icon: Icon, title, children }) => (
  <div>
    ...
    <div className="grid gap-2.5 grid-cols-1 sm:grid-cols-2">{children}</div>
  </div>
);

const TemplateSuratFilterModal = ({ filters, toggleFilter, onApply, onReset, onClose }) => {
  useEffect(() => {
      ...
      >
        ...
        <div className="relative overflow-hidden bg-gradient-to-r from-[#0B1442] via-[#101F5C] to-[#1E3A8A] px-6 py-6 shrink-0">
          ...
          <div className="relative flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-white">Filter Template Surat</h3>
              <p className="text-[11px] text-white/60 mt-0.5">Saring template sesuai peserta, status, dan aset</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <Section icon={BadgeCheck} title="Status">
            {statusOptions.map((o) => (
              <CheckboxItem key={o.key} label={o.label} checked={filters.status.includes(o.key)} onToggle={() => toggleFilter("status", o.key)} />
            ))}
          </Section>

          <Section icon={ImageIcon} title="Kelengkapan Aset">
            {asetOptions.map((o) => (
              <CheckboxItem key={o.key} label={o.label} checked={filters.aset.includes(o.key)} onToggle={() => toggleFilter("aset", o.key)} />
            ))}
          </Section>
        </div>
...
export default TemplateSuratFilterModal;