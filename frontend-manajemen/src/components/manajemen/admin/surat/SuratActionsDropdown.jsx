...
const SuratActionsDropdown = ({ sudahTerbit, sudahDikirim, onEdit, onDelete, onTerbitkan, onKirimEmail }) => {
    ...
    <>
      ...
      {open && createPortal(
        <div
          ...
          className={`w-54 rounded-xl border border-slate-200 bg-white shadow-2xl overflow-hidden transition-all duration-200 ${
            visible ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-90 -translate-y-2"
          }`}
        >
          {sudahTerbit ? (
              <>
                <button onClick={() => handleAction(onEdit)} className="group/item flex w-full items-center gap-3 px-4 py-3 text-left bg-slate-50/70 hover:bg-slate-100 transition-colors duration-200 cursor-pointer">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-[#0B1442]">Kelola Surat</p>
                    <p className="text-[10px] text-slate-400 whitespace-nowrap">Ubah data, lihat & unduh PDF</p>
                  </div>
                </button>
                <div className="border-t border-slate-100" />
                  {/* Kirim PDF surat ke email peserta (email saat pendaftaran).
                      Berguna untuk pengiriman ulang setelah surat diperbaiki
                      atau bila pengiriman otomatis gagal. */}
                  <button onClick={() => handleAction(onKirimEmail)} className="group/item flex w-full items-center gap-3 px-4 py-3 text-left bg-sky-50/40 hover:bg-sky-50/70 transition-colors duration-200 cursor-pointer">
                    <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-sky-100 text-sky-600 transition-all duration-200 group-hover/item:scale-110 group-hover/item:-rotate-6">
                      <MailCheck className="w-3.5 h-3.5" />
                    </span>
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-[#0B1442]">{sudahDikirim ? "Kirim Ulang Email" : "Kirim ke Email Peserta"}</p>
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-red-700">Hapus Surat</p>
                    <p className="text-[10px] text-slate-400 whitespace-nowrap">Hapus surat beserta PDF-nya</p>
                  </div>
                </button>
              </>
            ) : (
              <button onClick={() => handleAction(onTerbitkan)} className="group/item flex w-full items-center gap-3 px-4 py-3 text-left bg-emerald-50/40 hover:bg-emerald-50/70 transition-colors duration-200 cursor-pointer">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-emerald-100 text-emerald-600 transition-all duration-200 group-hover/item:scale-110 group-hover/item:-rotate-6">
                  <Plus className="w-3.5 h-3.5" />
                </span>
                <div className="min-w-0">
                  <p className="text-xs font-bold text-[#0B1442]">Terbitkan Surat</p>
