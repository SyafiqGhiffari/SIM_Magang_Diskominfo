...
const sortOptions = [
  { key: "terbaru", label: "Terbaru Masuk" },
  { key: "terlama", label: "Terlama Masuk" },
  { key: "paling_sering", label: "Paling Sering Ditanya" },
  { key: "skor_terendah", label: "Kecocokan FAQ Terendah" },
];

const PertanyaanSortDropdown = ({ sortBy, setSortBy }) => {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute left-0 top-11 z-20 w-60 rounded-xl border border-slate-200 bg-white shadow-xl overflow-hidden animate-[fadeslide_0.15s_ease-out]">
          {sortOptions.map((opt) => (
            <button
              key={opt.key}
              onClick={() => {
...
export default PertanyaanSortDropdown;