...
import { Filter as FilterIcon, X, ListFilter, RotateCcw, Check, Zap } from "lucide-react";
...
const jenisOptions = [
  { key: "quick_action", label: "Quick Action Saja" },
  { key: "publik", label: "Tampil di Halaman Publik" },
  { key: "perlu_perbaikan", label: "Perlu Ditulis Ulang" },
];
      ...
      >
        ...
        <div className="relative overflow-hidden bg-gradient-to-r from-[#0B1442] via-[#101F5C] to-[#1E3A8A] px-6 py-6 shrink-0">
          ...
          <div className="relative flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-white">Filter FAQ</h3>
              <p className="text-[11px] text-white/60 mt-0.5">Sesuaikan tampilan daftar FAQ sesuai kebutuhan Anda</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2.5">
              {jenisOptions.map((j) => (
                <CheckboxItem key={j.key} label={j.label} checked={jenisList.includes(j.key)} onToggle={() => toggleJenis(j.key)} />
              ))}
            </div>
          </div>
        </div>
