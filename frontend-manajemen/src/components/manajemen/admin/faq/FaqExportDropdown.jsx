...
// Ikon dokumen kustom bergaya "file explorer" dengan badge format —
// disamakan dengan ExportDropdown pada halaman Kelola Bidang.
...
const formats = [
  ...
  {
    key: "impor",
    label: "CSV Format Impor",
    desc: "Bisa disunting lalu diimpor lagi",
    icon: <FileIcon badge="SYNC" color="#7c3aed" bgColor="#ede9fe" />,
  },
];

/**
 * Varian ExportDropdown untuk kartu samping halaman FAQ.
 * Tombol dibuat selebar kartu, menu tetap memakai gaya yang sama
 * dengan halaman Kelola Bidang agar konsisten.
 */
const FaqExportDropdown = ({ onExport }) => {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((p) => !p)}
        className={`group inline-flex w-full items-center justify-center gap-2 rounded-xl border px-4 py-2.5 text-xs font-bold shadow-sm transition-all duration-200 cursor-pointer hover:-translate-y-0.5 hover:shadow-md active:scale-95 ${
          open
      ...
      <div
        className={`absolute right-0 top-12 z-30 w-72 origin-top-right rounded-2xl border border-slate-200 bg-white shadow-xl overflow-hidden transition-all duration-250 ${
...
export default FaqExportDropdown;