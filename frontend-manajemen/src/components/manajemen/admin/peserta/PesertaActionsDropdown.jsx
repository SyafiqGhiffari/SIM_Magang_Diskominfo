...
import { MoreVertical, Eye, KeyRound, Trash2 } from "lucide-react";

const PesertaActionsDropdown = ({ onDetail, onResetPassword, onDelete }) => {
  ...
  return (
    <>
      ...
      {open && createPortal(
        ...
        >
          <button onClick={() => handleAction(onDetail)} className="group/item flex w-full items-center gap-3 px-4 py-3 text-left bg-slate-50/70 hover:bg-slate-100 transition-colors duration-200 cursor-pointer">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-200 text-slate-600 transition-all duration-200 group-hover/item:scale-110 group-hover/item:-rotate-6">
              <Eye className="w-3.5 h-3.5" />
            </span>
            <div className="min-w-0">
              <p className="text-xs font-bold text-[#0B1442]">Detail Peserta</p>
              <p className="text-[10px] text-slate-400 whitespace-nowrap">Lihat data lengkap & dokumen</p>
            </div>
          </button>
          <div className="border-t border-slate-100" />
          <button onClick={() => handleAction(onResetPassword)} className="group/item flex w-full items-center gap-3 px-4 py-3 text-left bg-blue-50/40 hover:bg-blue-50/70 transition-colors duration-200 cursor-pointer">
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold text-[#0B1442]">Reset Password</p>
              <p className="text-[10px] text-slate-400 whitespace-nowrap">Kirim password baru ke email</p>
            </div>
          </button>
          <div className="border-t border-slate-100" />
          <button onClick={() => handleAction(onDelete)} className="group/item flex w-full items-center gap-3 px-4 py-3 text-left bg-red-50/40 hover:bg-red-50/70 transition-colors duration-200 cursor-pointer">
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold text-red-700">Hapus Akun</p>
              <p className="text-[10px] text-slate-400 whitespace-nowrap">Riwayat pendaftaran tetap tersimpan</p>
            </div>
          </button>
        </div>,
        document.body
...
export default PesertaActionsDropdown;