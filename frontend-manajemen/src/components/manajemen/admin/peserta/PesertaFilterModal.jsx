...
import { Filter as FilterIcon, X, ListFilter, RotateCcw, Check, Building2, ChevronDown, CalendarClock } from "lucide-react";
...
const CustomSelect = ({ value, onChange, options, placeholder }) => {
  ...
  // reposisi ulang saat scroll/resize supaya dropdown tetap menempel tombol
  useEffect(() => {
    if (!open) return;
    const reposition = () => {
      if (btnRef.current) {
      ...
      <div className="w-full max-w-lg rounded-3xl bg-white shadow-2xl overflow-hidden animate-[modalFadeUp_0.3s_ease-out] max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="relative overflow-hidden bg-gradient-to-r from-[#0B1442] via-[#101F5C] to-[#1E3A8A] px-6 py-6 shrink-0">
          ...
          <div className="relative flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-white">Filter Peserta</h3>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2.5">
              {periodeOptions.map((p) => (
                <CheckboxItem key={p.key} label={p.label} checked={periodeList.includes(p.key)} onToggle={() => togglePeriode(p.key)} />
              ))}
            </div>
